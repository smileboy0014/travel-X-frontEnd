...
const DetailCalendarModal = ({ isOpen, onRequestClose, availableDates }) => {
  ...
  const [value, onChange] = useState([new Date(detailDate.start), new Date(detailDate.end)]);
				...
				<div className={Style.CalenderPopHeader}>
					<div className='site-container'>
						<div className={Style.CalenderPopHeaderTitle}>
							<div className={Style.CalenderPopHeaderTitle_title}>날짜 선택</div>
							<button type="button" className={Style.CalenderPopHeaderTitle_close} onClick={() => onRequestClose(false)}><span className="ab-text">Close</span></button>
						</div>
              ...
              <Calendar
                ...
                showNeighboringMonth={false}
								tileDisabled={({date, view}) =>(view === 'month') &&
									availableDateList && availableDateList.some(availableDate => 
					...
					<div className='site-container'>
						<div className={Style.CalenderPopFooterCheck}>
							<dl className={Style.CalenderPopFooterCheck_item_Start}>
								<dt className={value[0] ? Style.CalenderPopFooterCheck_title : Style.CalenderPopFooter_title}>체크인</dt>
								{
									value[0] && <dd className={Style.CalenderPopFooterCheck_day}><span className={Style.CalenderPopFooterCheck_text}>{value[0].getMonth() + 1}.{value[0].getDate()}</span>({week[value[0].getDay()]})</dd>
								}
							</dl>
							<dl className={Style.CalenderPopFooterCheck_item_End}>
								<dt className={value[1] ? Style.CalenderPopFooterCheck_title : Style.CalenderPopFooter_title}>체크아웃</dt>
								{
									value[1] && <dd className={Style.CalenderPopFooterCheck_day}><span className={Style.CalenderPopFooterCheck_text}>{value[1].getMonth() + 1}.{value[1].getDate()}</span>({week[value[1].getDay()]})</dd>
								}
							</dl>
						</div>
						<div className={Style.CalenderPopFooterBtn}>
							{
								...
								<button 
									...
									onClick={() => { 
										dispatch(dateActions.setDetailDate({start: value[0].toJSON(), end: value[1].toJSON()}));
										onRequestClose(false);}}>
									{Math.ceil((value[1].getTime() - value[0].getTime()) / (1000*60*60*24))}박 선택 완료
								</button> :
								<div className={Style.CalenderPopFooterinfo}>날짜를 선택해 주세요</div>
							}
						</div>
					</div>
				</div>
