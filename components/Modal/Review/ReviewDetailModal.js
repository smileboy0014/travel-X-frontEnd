...
const ReviewDetailModal = (props) => {
	...
	const { data, isOpen, onRequestClose, point1, point2, point3, point4, point5, totalPoint, totalCount } = props;
				...
				<div className={style.modalBody}>
					...
					<div className='rightDetail'>
						...
						<div className='reviewContent'>
							{
								content ? content.map((review, index) => (<ReviewCard key={index} {...review} />))  
								:data.map(
									(review, index) => (<ReviewCard key={index} {...review} />)
								)
							}
						</div>
					</div>
